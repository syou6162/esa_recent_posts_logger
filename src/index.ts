...
async function getUpdatedPosts(
  ...
  const fromStr = formatDate(convertToTimeZone(from, { timeZone: timeZone }), 'yyyy-MM-dd')
  const toStr = formatDate(convertToTimeZone(to, { timeZone: timeZone }), 'yyyy-MM-dd')
...
getUpdatedPosts(axiosClient, esaConfig, yesterday, today).then((result: EsaSearchResult) => {
  const category = `日報/${formatDate(convertToTimeZone(yesterday, { timeZone: timeZone }), 'yyyy/MM/dd')}`
